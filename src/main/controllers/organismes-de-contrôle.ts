import { OrganismeDeControleFields, OrganismeDeControleTypes } from "@shared/model/organismes-de-controle";
...
export class OrganismesDeControleController extends BaseController<"organismesDeContrôle"> implements Crud<OrganismeDeControleTypes>, Hydrator<OrganismeDeControleTypes> {
  ...
  async remove(projectId: ProjectId, query: Filter<OrganismeDeControleFields>): Promise<void> {
    const project = Project.get(projectId)!;
    const organismes = project.db.getCollection("organismes-de-contrôle"); 
    organismes.remove(query)
  }
